...
export class EmployeeCalculate extends Component {
         ...
         : <div>
            <form>
               ...
               {this.state.employeeTypeId === 1 ?
                  <div className="form-row">
                     <div className='form-group col-md-12'><label>Salary: 20,000.00 </label></div>
                     <div className='form-group col-md-12'><label>Tax: 12% </label></div>
                  </div> : <div className="form-row">
                     <div className='form-group col-md-12'><label>Rate Per Day: 500.00 </label></div>
                  </div>}
               ...
               <div className="form-row">
                  <div className='form-group col-md-12'>
                     <label>Net Income: <b>{this.state.netIncome.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</b></label>
                  </div>
               </div>
   ...
   async getEmployee(id) {
      ...
      if (response.status === 200) {
         const data = await response.json();
         this.setState({ id: data.id, fullName: data.fullName, birthdate: format(data.birthdate, 'yyyy-MM-dd'), tin: data.tin, employeeTypeId: data.employeeTypeId, loading: false, loadingCalculate: false });
      }
      else {
